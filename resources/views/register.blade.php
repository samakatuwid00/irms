        ...
        <div id="flashContainer" class="fixed top-4 right-4 z-50 space-y-4 max-w-md w-full pointer-events-none">
            @if (session('success'))
                <div id="successMessage"
                    class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg shadow-lg flex items-center justify-between pointer-events-auto animate-fade-in">
                    <span>{{ session('success') }}</span>
                    <button type="button" onclick="closeFlash('successMessage')"
                        class="ml-4 text-green-800 hover:text-green-900 font-bold text-xl">&times;</button>
                </div>
            @endif

            @if (session('error'))
                <div id="errorMessage"
                    class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg shadow-lg flex items-center justify-between pointer-events-auto animate-fade-in">
                    <span>{{ session('error') }}</span>
                    <button type="button" onclick="closeFlash('errorMessage')"
                        class="ml-4 text-red-800 hover:text-red-900 font-bold text-xl">&times;</button>
                </div>
            @endif

            @if ($errors->any())
                <div id="validationErrors"
                    class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg shadow-lg pointer-events-auto animate-fade-in">
                    <div class="flex items-center justify-between mb-2">
                        <strong>Please fix the following errors:</strong>
                        <button type="button" onclick="closeFlash('validationErrors')"
                            class="text-red-800 hover:text-red-900 font-bold text-xl">&times;</button>
                    </div>
            ...
            <form method="POST" action="{{ route('register.submit') }}" id="registerForm" autocomplete="on">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" value="{{ old('firstname') }}" id="firstname" name="firstname" required
                            autocomplete="given-name" class="input-field">
                    </div>
                    <div>
                        {{-- FIX: added id + autocomplete --}}
                        <label for="lastname" class="block text-sm font-medium mb-1">Last Name</label>
                        <input type="text" value="{{ old('lastname') }}" id="lastname" name="lastname" required
                            autocomplete="family-name" class="input-field">
                    </div>
                    <div>
                        {{-- FIX: added id + autocomplete --}}
                        <label for="middlename" class="block text-sm font-medium mb-1">Middle Name</label>
                        <input type="text" value="{{ old('middlename') }}" id="middlename" name="middlename"
                            autocomplete="additional-name" class="input-field">
                    </div>
                    <div>
                        {{-- FIX: added id + autocomplete --}}
                        <label for="extension_name" class="block text-sm font-medium mb-1">Extension</label>
                        <select name="extension_name" id="extension_name" autocomplete="honorific-suffix"
                            class="input-field">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="date" value="{{ old('birthday') }}" id="birthday" name="birthday"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="username" name="username" value="{{ old('username') }}" required
                        placeholder="Username" autocomplete="username" class="input-field w-full">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email') }}" required
                        placeholder="Email" autocomplete="email" class="input-field w-full">
                </div>
                    ...
                    <div class="relative password-container">
                        ...
                        <div class="password-container">
                            <input type="password" id="password" name="password" required autocomplete="new-password"
                                class="w-full px-3 py-2 pr-10 border border-custom-teal rounded-md focus:outline-none focus:ring-2 focus:ring-custom-teal">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="authority_level" id="authority_level" autocomplete="off" class="input-field">
                            ...
                            <option value="4" {{ old('authority_level') == '4' ? 'selected' : '' }}>Regional Account
                            </option>
                            <option value="3" {{ old('authority_level') == '3' ? 'selected' : '' }}>Division Account
                            </option>
                            <option value="2" {{ old('authority_level') == '2' ? 'selected' : '' }}>District Account
                            </option>
                            <option value="1" {{ old('authority_level') == '1' ? 'selected' : '' }}>School Account
                            </option>
                        </select>
                    </div>
                    <div>
                ...
                <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                    <button type="submit" id="submitBtn"
                        class="w-full sm:w-1/2 bg-custom-yellow text-custom-dark py-2 rounded-md font-semibold hover:bg-custom-yellow-hover transition">Submit</button>
                    <button type="reset"
                        class="w-full sm:w-1/2 bg-gray-300 text-custom-dark py-2 rounded-md font-semibold hover:bg-gray-400 transition">Clear</button>
                </div>
