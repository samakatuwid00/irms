<div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Manage Districts and Schools</h1>
    </div>
    ...
    <div class="border-b border-gray-200 overflow-x-auto">
        <nav class="flex space-x-4 sm:space-x-8 min-w-max">
            <button type="button"
                    ...
                    id="tab-districts"
                    class="tab-button py-3 sm:py-4 px-2 sm:px-1 border-b-2 font-medium text-sm transition-colors
                           border-blue-500 text-blue-600 whitespace-nowrap">
                Districts
            </button>
            <button type="button"
    ...
    {{-- ================= DISTRICTS TAB ================= --}}
    <div id="content-districts" class="tab-content space-y-4">

        <!-- Search & Add District - Responsive Stack -->
        ...
        <!-- Districts Table - Responsive with overflow -->
        <div class="bg-white rounded-xl shadow overflow-hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm text-left">
                    <thead class="bg-gray-100 text-gray-600 uppercase text-xs">
                        <tr>
                            <th class="px-3 sm:px-4 py-3">District Name</th>
                            <th class="px-3 sm:px-4 py-3 hidden md:table-cell">Short Name</th>
                            <th class="px-3 sm:px-4 py-3 hidden lg:table-cell">Address</th>
                            <th class="px-3 sm:px-4 py-3 hidden xl:table-cell">Legislative District</th>
                            <th class="px-3 sm:px-4 py-3 hidden lg:table-cell">Contact</th>
                            <th class="px-3 sm:px-4 py-3 hidden xl:table-cell">Email</th>
                            <th class="px-3 sm:px-4 py-3 hidden 2xl:table-cell">Date Established</th>
                    ...
                    <tbody class="divide-y">
                        @forelse ($districts as $district)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 sm:px-4 py-3 font-medium text-sm">{{ $district->district_name }}</td>
                                <td class="px-3 sm:px-4 py-3 hidden md:table-cell">{{ $district->shortname ?? '—' }}</td>
                                <td class="px-3 sm:px-4 py-3 hidden lg:table-cell">{{ Str::limit($district->address ?? '—', 30) }}</td>
                                <td class="px-3 sm:px-4 py-3 hidden xl:table-cell">{{ $district->legislative_district ?? '—' }}</td>
                                <td class="px-3 sm:px-4 py-3 hidden lg:table-cell">{{ $district->contact_number ?? '—' }}</td>
                                <td class="px-3 sm:px-4 py-3 hidden xl:table-cell">{{ Str::limit($district->email ?? '—', 25) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-3 sm:px-4 py-8 text-center text-gray-500">
                                    No districts found.
                                </td>
                            </tr>
                        @endforelse
    ...
    {{-- ================= SCHOOLS TAB ================= --}}
    <div id="content-schools" class="tab-content hidden space-y-4">

        <!-- District Selector & Search - Responsive Grid -->
        <div class="bg-white rounded-xl shadow p-3 sm:p-4">
            <form method="GET" id="schoolFilterForm">
                <input type="hidden" name="active_tab" value="schools">

                <div class="flex flex-col gap-4">
                    <!-- District Dropdown - Full width on mobile -->
                    <div class="w-full">
                    ...
                    <div class="w-full">
                        ...
                        <div class="flex flex-col sm:flex-row gap-2">
                            ...
                            <div class="flex gap-2">
                                <button type="submit"
                                        class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium whitespace-nowrap">
        ...
        @if(request('selected_district'))
            ...
            <div class="bg-white rounded-xl shadow overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-4 sm:px-6 py-3 sm:py-4">
                    <h3 class="text-base sm:text-lg font-bold text-white">{{ $selectedDistrict->district_name }}</h3>
                    <p class="text-blue-100 text-xs sm:text-sm">{{ $schoolsToDisplay->count() }} school(s) found</p>
                </div>
                        ...
                        <table class="min-w-full text-sm text-left">
                            <thead class="bg-gray-50 text-gray-600 uppercase text-xs">
                                <tr>
                                    ...
                                    <th class="px-3 sm:px-4 py-3 hidden sm:table-cell">School ID</th>
                                    <th class="px-3 sm:px-4 py-3 hidden md:table-cell">Type</th>
                                    <th class="px-3 sm:px-4 py-3 hidden lg:table-cell">Short Name</th>
                                    <th class="px-3 sm:px-4 py-3 hidden xl:table-cell">Address</th>
                                    <th class="px-3 sm:px-4 py-3 hidden lg:table-cell">Contact</th>
                                    <th class="px-3 sm:px-4 py-3 hidden 2xl:table-cell">Email</th>
                                    <th class="px-3 sm:px-4 py-3 text-center">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y">
                                @foreach($schoolsToDisplay as $school)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-3 sm:px-4 py-3 hidden sm:table-cell">{{ $school->school_id ?? '—' }}</td>
                                        ...
                                        <td class="px-3 sm:px-4 py-3 hidden lg:table-cell">{{ $school->shortname ?? '—' }}</td>
                                        ...
                                        <td class="px-3 sm:px-4 py-3 hidden 2xl:table-cell">{{ Str::limit($school->email ?? '—', 25) }}</td>
                                        <td class="px-3 sm:px-4 py-3">
                                            <div class="flex justify-center gap-2 sm:gap-3">
                                                <!-- Edit -->
                                                <button type="button"
        ...
        @else
            <div class="bg-white rounded-xl shadow p-6 sm:p-8 text-center text-gray-500">
                ...
                <p class="text-base sm:text-lg font-medium">Please select a district to view schools</p>
            </div>
        @endif
    </div>
</div>

{{-- ADD DISTRICT MODAL - Responsive --}}
<div id="add-district-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-3 sm:p-4">
    <div class="bg-white rounded-2xl shadow-2xl p-5 sm:p-7 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h2 class="text-lg sm:text-xl font-bold mb-4 sm:mb-6">Add District</h2>
        ...
        <form action="{{ route('districts.store') }}" method="POST">
            ...
            <div class="mt-5 sm:mt-6 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                <button type="button" onclick="closeModal('add-district-modal')" class="px-4 py-2 bg-gray-200 rounded order-2 sm:order-1">
                    Cancel
                </button>
...
{{-- EDIT DISTRICT MODAL - Responsive --}}
<div id="edit-district-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-3 sm:p-4">
    <div class="bg-white rounded-2xl shadow-2xl p-5 sm:p-7 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h2 class="text-lg sm:text-xl font-bold mb-4 sm:mb-6">Edit District</h2>
        ...
        <form method="POST" id="editDistrictForm">
            ...
            <div class="space-y-2">
                ...
                <input id="edit_district_name" name="district_name" required class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Short Name</label>
                <input id="edit_shortname" name="shortname" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
                <input id="edit_address" name="address" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Legislative District</label>
                <input id="edit_legislative_district" name="legislative_district" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Contact Number</label>
                <input id="edit_contact_number" name="contact_number" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input id="edit_email" name="email" type="email" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Date Established</label>
                <input id="edit_date_establish" name="date_establish" type="date" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">
            </div>

            <div class="mt-5 sm:mt-6 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                <button type="button" onclick="closeModal('edit-district-modal')" class="px-4 py-2 bg-gray-200 rounded order-2 sm:order-1">
                    Cancel
                </button>
                <button class="px-4 py-2 bg-blue-600 text-white rounded order-1 sm:order-2">
                    Update
...
{{-- DELETE DISTRICT MODAL - Responsive --}}
<div id="delete-district-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-3 sm:p-4">
    <div class="bg-white rounded-xl p-5 sm:p-6 max-w-md w-full mx-3 sm:mx-0">
        <h2 class="text-base sm:text-lg font-bold text-red-600 mb-3 sm:mb-4">Delete District</h2>
        <p id="deleteDistrictMessage" class="mb-4 sm:mb-6 text-sm sm:text-base"></p>
        ...
        <form method="POST" id="deleteDistrictForm">
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                <button type="button" onclick="closeModal('delete-district-modal')" class="px-4 py-2 bg-gray-200 rounded order-2 sm:order-1">
                    Cancel
                </button>
        ...
        <form action="{{ route('schools.store') }}" method="POST" id="addSchoolForm">
            ...
            <div class="space-y-3 sm:space-y-4">
                <div>
                    ...
                    <select name="district_id" id="add_school_district_id" required class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">
                        <option value="">-- Select District --</option>
                        @if($districtsWithSchools)
                            @foreach($districtsWithSchools as $district)
                                <option value="{{ $district->id }}"
            ...
            <div class="mt-5 sm:mt-6 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                <button type="button" onclick="closeModal('add-school-modal')" class="px-4 py-2 bg-gray-200 rounded order-2 sm:order-1">
                    Cancel
                </button>
                <button class="px-4 py-2 bg-blue-600 text-white rounded order-1 sm:order-2">
                    Save
    ...
    <div class="bg-white rounded-2xl shadow-2xl p-5 sm:p-7 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form method="POST" id="editSchoolForm">
            ...
            <div class="space-y-2">
                ...
                <input id="edit_school_name" name="school_name" required class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">School Type</label>
                <select id="edit_school_type" name="school_type" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">
                    <option value="" selected disabled>Select School Type</option>
                    <option value="KINDER">Kinder Only</option>
                    <option value="ELEMENTARY">Elementary</option>
                    <option value="JHS">Junior High School</option>
                ...
                <label class="block text-sm font-medium text-gray-700 mb-1">Short Name</label>
                <input id="edit_school_shortname" name="shortname" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
                <input id="edit_school_address" name="address" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Contact Number</label>
                <input id="edit_school_contact_number" name="contact_number" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input id="edit_school_email" name="email" type="email" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Date Established</label>
                <input id="edit_school_date_establish" name="date_establish" type="date" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">

                <label class="block text-sm font-medium text-gray-700 mb-1">Legislative School</label>
                <input id="edit_school_legislative" name="legislative_school" class="w-full border rounded-lg px-4 py-2 text-sm sm:text-base">
            </div>

            <div class="mt-5 sm:mt-6 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                <button type="button" onclick="closeModal('edit-school-modal')" class="px-4 py-2 bg-gray-200 rounded order-2 sm:order-1">
                    Cancel
                </button>
                <button class="px-4 py-2 bg-blue-600 text-white rounded order-1 sm:order-2">
                    Update
...
<div id="delete-school-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-3 sm:p-4">
    <div class="bg-white rounded-xl p-5 sm:p-6 max-w-md w-full mx-3 sm:mx-0">
        ...
        <form method="POST" id="deleteSchoolForm">
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                <button type="button" onclick="closeModal('delete-school-modal')" class="px-4 py-2 bg-gray-200 rounded order-2 sm:order-1">
                    Cancel
                </button>
                <button class="px-4 py-2 bg-red-600 text-white rounded order-1 sm:order-2">
                    Delete
