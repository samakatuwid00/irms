    ...
    public function up()
    {
        ...
        DB::statement("
            ...
            BEGIN
                ...
                SELECT brand, code, version, url, size, model,
                ...
                FROM nonprint_types
                WHERE id = res_rec.nonprint_type_id;

                -- Build weighted search vector
                --   A → title          (highest relevance)
                --   B → brand, subjects
                --   C → code, model, grades, type_name, shortname
        ...
        echo "Populating search vectors for existing nonprint acquisitions (simple dictionary for grades)...\n";
        DB::statement('
